...
(function($) { "use strict";
  ...
  var EventHandler = function() {
    ...
    var insertImages = function(welEditable, imageinput) {
      welEditable.trigger('focus');
      if (callback.onImageUpload) {
          callback.onImageUpload(imageinput, editor, welEditable); // call custom handler
      } else {
        $.each(imageinput.files, function(idx, file) {
    ...
    var hToolbarAndPopoverClick = function(event) {
      ...
      if (welBtn.length > 0) {
        ...
        if ($.inArray(sEvent, ['backColor', 'foreColor']) !== -1) {
          ...
          dialog.showImageDialog(welDialog, hDropImage, function(imageinput) {
            insertImages(welEditable, imageinput);
          });
